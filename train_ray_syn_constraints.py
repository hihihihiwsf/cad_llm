...
def train_on_ray_cluster():
    ...
    datamodule = SynConstraintsDataModule(
        args=main_args,
        ray_args=ray_args
    )
    ...
    model_class = ByT5SynConstraintsModel
    data_class = SynConstraintsDataModule
    ...
    model = ByT5SynConstraintsModel(args=main_args, tokenizer=tokenizer)

    trainer = pl.Trainer(
        callbacks=call_backs,
        accelerator=main_args.accelerator,
        devices=main_args.devices,
        strategy=main_args.strategy,
        logger=loggers,
        max_epochs=main_args.epochs,
        log_every_n_steps=log_every_n_steps,
        val_check_interval=main_args.val_check_interval,
