...
class ByT5Model(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        image_for_llm = self.gelu(self.layernorm(self.mapper(input_image_features)))

        
        txt_encoder_output = self.model.encoder(batch['input_ids'], batch['attention_mask'],output_hidden_states=True).last_hidden_state
        ...
        txt_embeds = self.fusion_text_transformer(torch.cat((icl_encoded, txt_encoder_output), 1))

    
        output_embed = torch.concatenate((image_for_llm, txt_embeds), dim=1)
        ...
        if model_batch['labels'] is not None:
            ...
            loss = loss_fct(lm_logits.view(-1, lm_logits.size(-1)), model_batch['labels'].view(-1))
