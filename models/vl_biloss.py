...
class BiVLT5Model(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        txt_embeddings = self.model.encoder(**model_batch)
        txt_embeddings = txt_embeddings.last_hidden_state
        
        encoder_output = torch.concatenate((image_for_llm, txt_embeddings), dim=1)
        ...
        decoder_batch = {}
        decoder_batch['encoder_outputs'] = encoder_outputs
        att = batch['attention_mask']
        decoder_batch['attention_mask'] = torch.cat((torch.ones(att.shape[0], image_for_llm.shape[1]).to(att.device), att), dim=1)
        ...
        ids_restore = torch.argsort(ids_shuffle, dim=1).to(hidden_state.device)
        
        vis_decoder_output = self.vis_model.decoder(hidden_state ,ids_restore=ids_restore)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        decoder_batch['attention_mask'] = torch.cat((torch.ones(att.shape[0], image_for_llm.shape[1]).to(self.device), att), dim=1)
        decoder_batch['labels'] = batch['labels']
