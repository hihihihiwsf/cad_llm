...
class ByT5Model(pl.LightningModule):
    ...
    def shift_right(self, input_ids, decoder_start_token_id, pad_token_id):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # txt_embeddings = self.model.encoder.get_input_embeddings()(batch['input_entities'].input_ids)
        ...
        new_batch = {}
        new_batch['input_ids'] = batch['input_entities'].input_ids
        new_batch['attention_mask'] = batch['input_entities'].attention_mask
        ...
        # model_batch = {}
        # ie = txt_embeddings.unsqueeze(1)
        # model_batch['inputs_embeds'] = ie
        # model_batch['labels'] = batch['output_entities'].input_ids
        # loss = self.model(**model_batch).loss
        ...
        tgt = self.model.encoder.get_input_embeddings()(o) # ignoring "C"
        ...
        batch_input_ids, batch_encoder_hidden = [], []
        indices = np.append(0, np.cumsum(l))
        for i in range(1, len(indices)):
            batch_input_ids.append(o[indices[i-1]:indices[i], :].reshape(1, -1))
            batch_encoder_hidden.append(unpacked_entities[indices[i-1]:indices[i], 0, :].unsqueeze(0))
            
        
        batch_2 = {}
        batch_2['encoder_hidden_states'] = outputs["decoder_hidden_states"][-1]
        batch_2['encoder_attention_mask'] = self.create_attention_mask_from_seq_length(l).to(self.device)
        batch_2['input_ids'] = self.shift_right(batch['labels'], self.model.config.decoder_start_token_id, self.model.config.pad_token_id)
        batch_2['input_ids'][batch_2['input_ids'] == -100] = self.model.config.pad_token_id
    ...
    def validation_step_12(self, batch, batch_idx):
        cols = ["input_ids", "attention_mask", "labels"]
        model_batch = {col: val for col, val in batch.items() if col in cols}
        txt_embeddings = self.model.encoder.get_input_embeddings()(batch['input_entities'].input_ids)
        #torch attention masking is oppostie of huggingface
        ...
        self.seq_inference(model_batch, batch)
        ...
        # loss = torch.nn.functional.cross_entropy(outputs.permute(0, 2, 1), lbl)
        lbl[lbl == self.tokenizer.pad_token_id] = -100
        loss = torch.nn.functional.cross_entropy(outputs.permute(0, 2, 1), lbl, ignore_index=-100)
    ...
    def validation_step_34(self, batch, batch_idx):
        ...
        txt_embeddings = txt_embeddings[:, 1, :]
        pad_embed = self.model.encoder.get_input_embeddings()(torch.tensor([self.tokenizer.pad_token_id]).to(self.device))
        start_embed = self.model.encoder.get_input_embeddings()(torch.tensor([self.tokenizer.bos_token_id]).to(self.device))
        ########### INPUT CHUNKS
        # Find the maximum chunk size
        chunks = batch['input_ent_length']
    ...
    def seq_inference(self, model_batch, batch, batch_idx=None):
        ...
        final_out = self.model.generate(**batch_final, output_hidden_states=True, return_dict_in_generate=True, max_new_tokens=192)
